...
class Mantenimiento extends MY_Controller {
    ...
    public function correctivo() {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($this->input->post('ciclo') == 1) {
                ...
                if ($sp_mensaje == 'OK_INGRESADO') {
                    $equipo = $this->mantencion_model->buscar_orden_model($id);
                    $data = array('equipo' => $equipo, 'success' => TRUE, 'contenido' => 'ot/orden/correctivo');
                }
            }
        }
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            ...
            $data = array('equipo' => $equipo, 'ciclo' => '1', 'success' => FALSE, 'contenido' => 'ot/orden/correctivo');
        }
        $this->load->view('ot/index', $data);
    }
    ...
    public function programasemana() {
        $data['contenido'] = 'ot/orden/programa_semanal2';
        $this->load->view('ot/index', $data);
    }
    ...
    public function agregar_repuestos() {
        $id = $this->uri->segment(3);
        if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            $data = array('contenido' => 'ot/orden/agregar_repuestos', 'titulo' => 'Agregar repuestos a la OT #' . $id, 'id' => $id);
            $this->load->view($this->template, $data);
        }

        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($this->input->post('componente') == 1) {
                $idEquipo = $this->input->post('id-equipo');
                $repuestos = $this->input->post('repuestos');
                $codigoEquipo = $this->input->post('codigo-equipo');
