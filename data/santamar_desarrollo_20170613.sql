...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 13-06-2017 a las 06:01:15
...
DROP PROCEDURE IF EXISTS `sp_ot_actualizar_producto`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_ot_actualizar_producto`(IN `idProductoParam` INT(8), IN `idCategoriaParam` INT(11), IN `codigoParam` VARCHAR(100), IN `productoParam` VARCHAR(100), IN `idBodegaParam` INT(2), IN `cantidadParam` INT(11), IN `cantidadMinimoParam` INT(4), IN `precioParam` INT(8))
BEGIN
    DECLARE cuenta INT(2);
    SET cuenta = (SELECT COUNT(1) FROM inventario_producto WHERE producto = productoParam OR codigo = codigoParam);
    IF cuenta > 0 THEN
        SELECT 'REGISTRO_EXISTE' as mensaje;
    ELSE
        ...
        cantidad_minimo = cantidadMinimoParam,
        precio = precioParam,
        fecha_registro = NOW()
        WHERE id_producto = idProductoParam;       
        SET @id = ROW_COUNT();
        IF @id > 0 THEN
            SELECT 'ACTUALIZAR_OK' as mensaje;
        ELSE
...
DROP PROCEDURE IF EXISTS `sp_ot_buscar_producto_id`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_ot_buscar_producto_id`(IN `idProductoParam` INT(8))
    NO SQL
...
WHERE P.id_producto = idProductoParam
ORDER BY P.producto$$
...
(3, 1, 'Código1', 'Producto1', 1, 2, 4, 3, '2017-06-08 23:15:35', 1),
(4, 1, 'kkk', 'ppp', 1, 1, 3, 2, '2017-06-05 22:30:49', 1),
(5, 1, 'kkk', 'pppoooooooo', 1, 1, 3, 2, '2017-06-05 22:49:49', 1),
(6, 1, 'ABC1', 'RODAMIENTO 1', 6, 1, 3, 2, '2017-06-08 23:47:30', 1),
(7, 1, 'ALFA99', 'TALADRO JUEVE99', 5, 9, 6, 7000, '2017-06-09 20:57:03', 1),
...
(11, 5, 'PLA_8', 'azul', 1, 2, 4, 3, '2017-06-06 10:55:28', 1);
...
(1, 'MAQUINA INNOVA 3', 'abc1', 1, '2017-06-07 22:46:09', 1),
(5, 'MAQUINA INNOVA', 'Bodega para almacenar repuestos y piezas de maquinaria.', 2, '2017-06-07 22:51:26', 0),
(6, 'MAQUINA INNOVA 2', 'abc1Descripcion', 1, '2017-06-07 22:45:49', 1);
...
(52, 24, '2017-06-13 00:00:00', '0000-00-00 00:00:00', 'CORRECTIVA', '', '', 1, 2, 1, '2017-06-12 23:19:35', 0, '00000', 1);
