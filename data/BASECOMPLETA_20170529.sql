...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-05-2017 a las 05:43:45
...
CREATE DEFINER=`santamar`@`localhost` PROCEDURE `sp_ot_actualizar_equipo`(IN `var_id_equipo` INT(3), IN `var_id_plan` INT(3), IN `var_id_area` INT(3), IN `var_id_subarea` INT(3), IN `var_tipo` CHAR(1), IN `var_equipo_actividad` VARCHAR(80), IN `var_id_responzable` INT(3), IN `var_observacion` VARCHAR(200))
    NO SQL
UPDATE mantencion_equipo_actividad
SET     
    id_plan = var_id_plan,
    id_area = var_id_area,
    id_sub_area = var_id_subarea,
    tipo = var_tipo,
...
ON B.id_plan = D.id_plan
WHERE A.id_orden = p_id_orden$$
...
CREATE DEFINER=`santamar`@`localhost` PROCEDURE `sp_ot_buscar_semana_mantencion`(IN `p_id_equipo` INT(3), IN `p_semana` VARCHAR(5))
    NO SQL
SELECT
A.id_plan,
B.id_area,
...
CREATE DEFINER=`santamar`@`localhost` PROCEDURE `sp_ot_crear_area`(IN `var_area` VARCHAR(50))
    NO SQL
BEGIN
...
-- MODIFICACION SP CREAR AREA
DELIMITER ;;
CREATE  PROCEDURE sp_ot_crear_area(IN `var_area` VARCHAR(50))
BEGIN
    DECLARE cuenta INT(8);
    SET cuenta = (SELECT COUNT(1) FROM mantencion_area WHERE area = var_area);
    IF cuenta > 0 THEN
...
INSERT INTO `inventario_categoria` (`id_categoria`, `categoria`) VALUES
(1, 'Computación'),
(2, 'Repuestos Maquinaría');
...
INSERT INTO `inventario_producto` (`id_producto`, `id_categoria`, `producto`, `bodega`, `cantidad`) VALUES
(1, 1, 'Teclado HP modelo Alfa 1', 'Bodega 1', 5);
...
(9, 35, 19, 2017, 'SEM09', 0, 'Revisión correa auxiliar de alternador.', '2017-05-29 21:00:20'),
(10, 35, 19, 2017, 'SEM14', 0, 'Revisión correa auxiliar de alternador.', '2017-05-29 21:00:20'),
(11, 35, 19, 2017, 'SEM18', 0, 'Revisión correa auxiliar de alternador.', '2017-05-29 21:00:20'),
